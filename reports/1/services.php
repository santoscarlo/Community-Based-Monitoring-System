...
// HEADER TITLE
    ...
    $this->CELL(0,0,'Maintenance',0,0,'C');
    $this->Ln();
...
function headerTable(){
 ...
 $this->Cell(40,10,'Request',1,0,'C');
 $this->Cell(40,10,'Aircon Name',1,0,'C');
 $this->Cell(30,10,'Complaints',1,0,'C');
...
function viewTable($db){
    $this->SetFont('Times','',12);
    $stmt = $db->query('SELECT * from staff_comments');
    while($data = $stmt->fetch(PDO::FETCH_OBJ)){
    $this->Cell(50,10,$data->lastname.', '.$data->firstname,1,0,'L');
    $this->Cell(40,10,$data->mobile,1,0,'L');
    $this->Cell(40,10,$data->request,1,0,'L');
    $this->Cell(40,10,$data->a_name,1,0,'L');
    $this->Cell(30,10,$data->complaint,1,0,'L');
...
// TOTAL RECORD COUNT
          ...
          $sql="SELECT count(*) from staff_comments";
